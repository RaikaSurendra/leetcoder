...
/**
 * Test suite for External Sourced Optimized solution
 * Validates correctness and demonstrates performance characteristics
 */
public class ExternalSourcedOptimizedTest {
    
    private final ExternalSourcedOptimized solution = new ExternalSourcedOptimized();
    ...
    public void testEdgeCaseSingleElement() {
        ...
        long expected = -5L;
        long actual = solution.maxMatrixSum(matrix);
        assertEquals(expected, actual, "Single negative element: must stay negative");
    }
    
    @Test
    public void testEdgeCaseSinglePositive() {
        int[][] matrix = {{42}};
        long expected = 42L;
        long actual = solution.maxMatrixSum(matrix);
        assertEquals(expected, actual, "Single positive element: stays positive");
    }
}
